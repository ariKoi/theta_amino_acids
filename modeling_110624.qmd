...
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed outliers. Here the algorithm reports a single outlier due to its parameterization, however this is a weak outlier since its deviation from the straight line is not large." 
...
```{r}
#| label: fig-Methionine-nonlinearities 
#| fig-cap: "Spearman's test for assessing non-linear relationships with the response. The further away from 0 the adjusted $\\rho^2$ score is, the higher the degree of non-linearity for that covariate." 
plot(spearman2(Methionine ~ sex + age + bmi + chronic_disease_flag + chronic_disease_v3 + meds_flag + supplements_flag + aerobics_activity + resistance_activity, data = datMethionine, p = 2, na.action = na.omit))
abline(v = c(-0.01/3, 0.01/3), lty = "dashed")
grid(ny = 9, nx = NA, col = "black")
```
...
```{r}
#| label: fig-Methionine-resid-1stpass 
#| fig-cap: "Figure combining a histogram with a density plot, a boxplot and the plot of the empirical distribution function (ECDF). The scale for the x-axis is synchronized over all plots. The median can be found on the boxplot as also in the ecdf-plot. The maximum and the minimum value are tagged with a tiny vertical dash upon the ecdf-line. The mean is shown in the boxplot as grey cross, the grey bar is its confidence interval." 

thematic::thematic_off()      
DescTools::Desc(resid_Methionine_mod, plotit = TRUE, digits = 2, main = "Residual Distribution - Methionine Model")
```

In comparison to the previous outlier analysis as seen [here](https://drive.google.com/file/d/1oJFw9-542I8KJnnaQdFjSxOrhZqxV7fA/view?usp=drive_link), the difference now is that the analysis is conducted on the model residuals, hence on values that are conditional on meta data (predictor variables) whereas previously the analysis was marginally or unconditionally executed with the only assumption being that of the amino acid concentration following a log-normal distribution. 

@fig-Methionine-outliers shows a quantile-quantile plot where the quantiles of the model's residual values are matched to the ones predicted from a normal distribution. The diagonal line indicates the ideal fit. Data points marked in red denote outliers (also indicated in the table below each plot with their corresponding subject IDs). 

For the rest of the figures below (i.e. for other amino acids), the same outlier detection procedure is applied but with a different parameterization which controls the outlier detection thresholds (i.e. horizontal lines in plot). 

```{r}
#| label: fig-Methionine-outliers 
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed outliers." 

# thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
out_resid_Methionine_mod$outliersPlot
```
...
```{r}
gt(out_resid_Methionine_mod$outliers) |> 
  tab_header(
    title = md("**Outliers**")
  ) |>  
  fmt_number(columns = Methionine) |> 
  tab_footnote(
    footnote = "Red coloured data points in above QQ plot",
    locations = cells_column_labels(columns = id)
  ) |> 
  tab_footnote(
    footnote = "μmol/g (model residual)",
    locations = cells_column_labels(columns = Methionine)
  ) |> 
  tab_options(table.background.color = "lavenderblush3", 
              table.font.size = "12px", 
              table.font.style = "oblique") |> 
...
```{r}
#| label: fig-Methionine-nonlinearities-2 
#| fig-cap: "Spearman's test for assessing non-linear relationships with the response. The further away from 0 the adjusted $\\rho^2$ score is, the higher the degree of non-linearity for that covariate." 
plot(spearman2(Methionine ~ sex + age + bmi + chronic_disease_flag + chronic_disease_v3 + meds_flag + supplements_flag + aerobics_activity + resistance_activity, data = datMethionine_refit, p = 2, na.action = na.omit))
abline(v = c(-0.01/3, 0.01/3), lty = "dashed")
grid(ny = 9, nx = NA, col = "black")
```
...
```{r}
#| label: fig-Methionine-linearities-2 
#| fig-cap: "Spearman's test for assessing linear relationships with the response. The further away from 0 the adjusted $\\rho^2$ score is, the higher the degree of linearity for that covariate." 
plot(spearman2(Methionine ~ sex + age + bmi + chronic_disease_flag + chronic_disease_v3 + meds_flag + supplements_flag + aerobics_activity + resistance_activity, data = datMethionine_refit, p = 1, na.action = na.omit))
abline(v = c(-0.01/3, 0.01/3), lty = "dashed")
grid(ny = 9, nx = NA, col = "black")
```
...
```{r}
#| label: fig-Methionine-refit-resid-qq 
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed weak outliers (the strong ones detected by the 1st pass model have been removed)." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
out_resid_Methionine_mod_refit$outliersPlot
```

```{r}
#| label: fig-Methionine-refit-resid-yhatVsResid
#| fig-cap: "Plot of predictions vs model residuals. No obvious patterns can be seen." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(fitted(Methionine_mod_refit), resid_Methionine_mod_refit, xlab = "Predicted", ylab = "Model Residuals"); abline(h=0)  # yhat vs. r
```

### Variable Importance

```{r}
#| label: fig-Methionine-refit-var-importance
#| fig-cap: "Proportion of overall model $R^2$ that is due to each predictor." 
#| fig-align: left 
#| fig-width: 8   

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(anova(Methionine_mod_refit), what = "proportion R2", main = "", sort = "ascending", margin = "") 
grid(ny = 15, nx = NA, col = "black")
...
<thead>
<tr><td colspan='8' style='text-align: left;'>
Effects          &emsp;&emsp;Response: <code>log(Methionine)</code></td></tr>
<tr><th style='border-bottom: 1px solid grey; font-weight: 900; border-top: 2px solid grey; text-align: center;'></th>
<th style='font-weight: 900; border-bottom: 1px solid grey; border-top: 2px solid grey; text-align: right;'>Low</th>
<th style='font-weight: 900; border-bottom: 1px solid grey; border-top: 2px solid grey; text-align: right;'>High</th>
<th style='font-weight: 900; border-bottom: 1px solid grey; border-top: 2px solid grey; text-align: right;'>&Delta;</th>
...
<td style='padding-left:4ex; text-align: right;'> 53.0</td>
<td style='padding-left:4ex; text-align: right;'> 19.0</td>
<td style='padding-left:4ex; text-align: right;'> 0.08486</td>
<td style='padding-left:4ex; text-align: right;'>0.14380</td>
<td style='padding-left:4ex; text-align: right;'>-0.1990</td>
<td style='padding-left:4ex; text-align: right;'> 0.36880</td>
</tr>
<tr>
<td style='text-align: left;'>bmi</td>
<td style='padding-left:4ex; text-align: right;'>22.7</td>
<td style='padding-left:4ex; text-align: right;'> 29.1</td>
<td style='padding-left:4ex; text-align: right;'>  6.4</td>
<td style='padding-left:4ex; text-align: right;'>-0.06711</td>
<td style='padding-left:4ex; text-align: right;'>0.08255</td>
<td style='padding-left:4ex; text-align: right;'>-0.2301</td>
<td style='padding-left:4ex; text-align: right;'> 0.09588</td>
</tr>
<tr>
<td style='text-align: left;'>resistance_activity</td>
<td style='padding-left:4ex; text-align: right;'> 0.0</td>
<td style='padding-left:4ex; text-align: right;'>450.0</td>
<td style='padding-left:4ex; text-align: right;'>450.0</td>
<td style='padding-left:4ex; text-align: right;'> 0.02033</td>
<td style='padding-left:4ex; text-align: right;'>0.17140</td>
<td style='padding-left:4ex; text-align: right;'>-0.3180</td>
<td style='padding-left:4ex; text-align: right;'> 0.35870</td>
</tr>
<tr>
<td style='text-align: left;'>sex --- Male:Female</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  2.0</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'>-0.27340</td>
<td style='padding-left:4ex; text-align: right;'>0.09941</td>
<td style='padding-left:4ex; text-align: right;'>-0.4697</td>
<td style='padding-left:4ex; text-align: right;'>-0.07713</td>
</tr>
<tr>
<td style='border-bottom: 2px solid grey; text-align: left;'>supplements_flag --- Yes:No</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>  2.0</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'></td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 0.02245</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>0.10230</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.1796</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 0.22450</td>
</tr>
</tbody>
</table>

```{r}
#| label: fig-Methionine-refit-var-effects
#| fig-cap: "The further away the central estimate and its corresponding intervals from unity the higher the variable's effect. Confidence intervals are 90%, 95% and 99%, with the dark blue denoting the 90% confidence interval." 
#| fig-width: 12  

thematic::thematic_off() 
plot(s_Methionine_mod_refit, digits = 1, log = TRUE, main = paste0("% increase/decrease on Methionine reference value of ", round(ddMethionine_refit$limits["Methionine"][2,]), " μmol/g (median)"), nb = 7)
```
...
# options(prType="html") 
# options(width = 164) 
rms:::html.validate(Methionine_mod_refit_val, digits = 2, caption = "2nd pass model validation")
```
...
<td style='border-bottom: 2px solid grey; text-align: center;'>1</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>1</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.59</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.41</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.59</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>300</td>
</tr>
</tbody>
...
```{r}
#| label: fig-Methionine-refit-cal
#| fig-cap: "Calibration plot for 2nd pass model. Deviation from the diagonal line after correcting for overfitting or bias." 
#| fig-width: 10
 
thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(Methionine_mod_refit_cal)
```
...
```{r eval=FALSE}
noquote(reg_f_Methionine_refit2)
```
...
```{r}
#| label: fig-Methionine-refit2-resid-qq 
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed outliers. Here the algorithm reports outliers due to its parameterization, however these are weak outliers since their deviation from the straight line is not large." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
out_resid_Methionine_mod_refit2$outliersPlot
```

```{r}
#| label: fig-Methionine-refit2-resid-yhatVsResid
#| fig-cap: "Plot of predictions vs model residuals. No obvious patterns can be observed." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(fitted(Methionine_mod_refit2), resid_Methionine_mod_refit2, xlab = "Predicted", ylab = "Model Residuals"); abline(h=0)  # yhat vs. r
```

### Variable Importance

```{r}
#| label: fig-Methionine-refit2-var-importance
#| fig-cap: "Proportion of overall model $R^2$ that is due to each predictor." 
#| fig-align: left 
#| fig-width: 8   

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(anova(Methionine_mod_refit2), what = "proportion R2", main = "", sort = "ascending", margin = "") 
grid(ny = 5, nx = NA, col = "black")
```
...
<td style='padding-left:4ex; text-align: right;'>53.0</td>
<td style='padding-left:4ex; text-align: right;'>19.0</td>
<td style='padding-left:4ex; text-align: right;'>-0.031860</td>
<td style='padding-left:4ex; text-align: right;'>0.12240</td>
<td style='padding-left:4ex; text-align: right;'>-0.2734</td>
<td style='padding-left:4ex; text-align: right;'> 0.20960</td>
</tr>
<tr>
<td style='text-align: left;'>bmi</td>
<td style='padding-left:4ex; text-align: right;'>22.7</td>
<td style='padding-left:4ex; text-align: right;'>29.1</td>
<td style='padding-left:4ex; text-align: right;'> 6.4</td>
<td style='padding-left:4ex; text-align: right;'> 0.005299</td>
<td style='padding-left:4ex; text-align: right;'>0.05805</td>
<td style='padding-left:4ex; text-align: right;'>-0.1092</td>
<td style='padding-left:4ex; text-align: right;'> 0.11980</td>
</tr>
<tr>
<td style='border-bottom: 2px solid grey; text-align: left;'>sex --- Male:Female</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 2.0</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'></td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.211300</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>0.07183</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.3530</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.06956</td>
</tr>
</tbody>
</table>

```{r}
#| label: fig-Methionine-refit2-var-effects
#| fig-cap: "The further away the central estimate and its corresponding intervals from unity the higher the variable's effect. Confidence intervals are 90%, 95% and 99%, with the dark blue denoting the 90% confidence interval." 
#| fig-width: 12  

thematic::thematic_off() 
plot(s_Methionine_mod_refit2, digits = 1, log = TRUE, main = paste0("% increase/decrease on Methionine reference value of ", round(ddMethionine_refit$limits["Methionine"][2,]), " μmol/g (median)"), nb = 4) # nb = 4 reduces the spacing between the CIs, looks better without it on the site
```

### Partial Dependence Plots

```{r}
#| label: fig-Methionine-refit2-var-partial-effects-plot1
#| fig-cap: "Effect of sex. Methionine concentration levels can be seen to be higher on average in females than in males." 
#| fig-width: 8  

ggplot(Predict(Methionine_mod_refit2, sex, fun = exp))
```

```{r}
#| label: fig-Methionine-refit2-var-partial-effects-plot2
#| fig-cap: "Non-Linear effect of age. Age is seen to decrease the levels of Methionine concentration on average."
#| fig-width: 8 

ggplot(Predict(Methionine_mod_refit2, age, fun = exp))
```

```{r}
#| label: fig-Methionine-refit2-var-partial-effects-plot4
#| fig-cap: "Average effect of bmi on levels of Methionine concentration can be seen to have an increasing impact for bmi levels of 35 and above (not significantly though)."
#| fig-width: 8 

ggplot(Predict(Methionine_mod_refit2, bmi, fun = exp))
```

```{r}
#| label: fig-Methionine-refit2-var-partial-effects-plot5
#| fig-cap: "Simultaneous effects of age and sex. Methionine concentration levels are seen to increase on average with decreasing both and particularly for females."
#| fig-width: 8  
  
# pred_intx1_Methionine is defined in workspace
bplot(pred_intx1_Methionine, yhat ~ age + sex, lfun = lattice::wireframe, zlab = "Methionine\n",  screen=list(z=120, x=-75), cex.lab = 0.55, cex.adj = 0.6)
```

```{r}
#| label: fig-Methionine-refit2-var-partial-effects-plot6
#| fig-cap: "Simultaneous effects of age and bmi. Methionine concentration levels are seen to increase on average with decreasing both bmi (less than 30) and age (less than 30)."
#| fig-width: 8  
  
# pred_intx2_Methionine is defined in workspace
bplot(pred_intx2_Methionine, yhat ~ age + bmi, lfun = lattice::wireframe, zlab = "Methionine\n",  screen=list(z=220, x=-75), cex.lab = 0.55, cex.adj = 0.6)
```

```{r}
#| label: fig-Methionine-refit2-var-partial-effects-plot7
#| fig-cap: "Simultaneous effects of bmi and sex. Methionine concentration levels are seen to increase on average especially for females."
#| fig-width: 8  
  
# pred_intx3_Methionine is defined in workspace
bplot(pred_intx3_Methionine, yhat ~ sex + bmi, lfun = lattice::wireframe, zlab = "Methionine\n",  screen=list(z=220, x=-75), cex.lab = 0.55, cex.adj = 0.6)
```
...
# options(prType="html") 
# options(width = 164) 
rms:::html.validate(Methionine_mod_refit2_val, digits = 2, caption = "3rd pass model validation")
```
...
<td style='text-align: center;'>0</td>
<td style='padding-left:3ex; text-align: center;'>0</td>
<td style='padding-left:3ex; text-align: center;'>0.41</td>
<td style='padding-left:3ex; text-align: center;'>-0.41</td>
<td style='padding-left:3ex; text-align: center;'>0.41</td>
<td style='padding-left:3ex; text-align: center;'>300</td>
</tr>
...
<td style='border-bottom: 2px solid grey; text-align: center;'>1</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>1</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.73</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.27</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.73</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>300</td>
</tr>
</tbody>
...
```{r}
#| label: fig-Methionine-refit2-cal
#| fig-cap: "Calibration plot for 3rd pass model. Deviation from the diagonal line after correcting for overfitting or bias. It can be seen that deviations have been reduced compared to the 2nd pass model, especially on the right side where the model is overestimating the actual concentration values." 
#| fig-width: 10
 
thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(Methionine_mod_refit2_cal)
```
...
dynRI <- new_dat |>
  nest(.by = id) |>
  mutate(preds = map(data, function(df) predict_f(Methionine_mod_refit2, new_dat = df, conf_type = "mean"))) |>
  unnest_wider(col = preds, names_repair = function(x) {
    nam <- x
    nam[3:4] <- paste0(nam[3:4], "RI")
    nam
  ...
  select(-data) |>
  inner_join(new_dat |> select(id, age, sex, bmi), by = "id") |>
  mutate(across(where(is.double), ~ round(., 2)))
...
```{r}
#| label: fig-Arginine-nonlinearities 
#| fig-cap: "Spearman's test for assessing non-linear relationships with the response. The further away from 0 the adjusted $\\rho^2$ score is, the higher the degree of non-linearity for that covariate." 
plot(spearman2(Arginine ~ sex + age + bmi + chronic_disease_flag + chronic_disease_v3 + meds_flag + supplements_flag + aerobics_activity + resistance_activity, data = datArginine, p = 2, na.action = na.omit))
abline(v = c(-0.01/3, 0.01/3), lty = "dashed")
grid(ny = 9, nx = NA, col = "black")
```

Non-linearity evident in sex, resistance_activity, chronic_disease_v3 and age (smaller $\rho^2$ with opposite sign). Some linear association is present with bmi, hence this covariate is also considered for the model.

To capture non linearities, interactions amongst these 6 variables will be considered, as well as spline transformations. The model achieving the best global significance (p-value = `0.0151`) contained the following terms: 

`log(Arginine) ~ sex + rcs(bmi,4) + rcs(age,4) + log(resistance_activity+1) + supplements_flag + chronic_disease_v3 + rcs(bmi,4):supplements_flag + rcs(age,4):log(resistance_activity+1) + log(resistance_activity+1):supplements_flag`
...
```{r}
#| label: fig-Arginine-resid-1stpass 
#| fig-cap: "Figure combining a histogram with a density plot, a boxplot and the plot of the empirical distribution function (ECDF). The scale for the x-axis is synchronized over all plots. The median can be found on the boxplot as also in the ecdf-plot. The maximum and the minimum value are tagged with a tiny vertical dash upon the ecdf-line. The mean is shown in the boxplot as grey cross, the grey bar is its confidence interval." 

thematic::thematic_off()      
DescTools::Desc(resid_Arginine_mod, plotit = TRUE, digits = 2, main = "Residual Distribution - Arginine Model")
```

In comparison to the previous outlier analysis as seen [here](https://drive.google.com/file/d/1oJFw9-542I8KJnnaQdFjSxOrhZqxV7fA/view?usp=drive_link), the difference now is that the analysis is conducted on the model residuals, hence on values that are conditional on meta data (predictor variables) whereas previously the analysis was marginally or unconditionally executed with the only assumption being that of the amino acid concentration following a log-normal distribution. 

@fig-Arginine-outliers shows a quantile-quantile plot where the quantiles of the model's residual values are matched to the ones predicted from a normal distribution. The diagonal line indicates the ideal fit. Data points marked in red denote outliers (also indicated in the table below each plot with their corresponding subject IDs). 

For the rest of the figures below (i.e. for other amino acids), the same outlier detection procedure is applied but with a different parameterization which controls the outlier detection thresholds (i.e. horizontal lines in plot). 

```{r}
#| label: fig-Arginine-outliers 
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed outliers." 

# thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
out_resid_Arginine_mod$outliersPlot
```

```{r}
gt(out_resid_Arginine_mod$outliers) |> 
  tab_header(
    title = md("**Outliers**")
  ) |>  
  fmt_number(columns = Arginine) |> 
  tab_footnote(
    footnote = "Red coloured data points in above QQ plot",
    locations = cells_column_labels(columns = id)
  ) |> 
  tab_footnote(
    footnote = "μmol/g (model residual)",
    locations = cells_column_labels(columns = Arginine)
  ) |> 
  tab_options(table.background.color = "lavenderblush3", 
              table.font.size = "12px", 
              table.font.style = "oblique") |> 
...
Here the model is refitted to the data excluding the strong outlier value detected in the previous step. The model formula is the same as the 1st pass. The global regression significance is characterized by a p-value of `0.0086`. 

### Residuals

```{r}
#| label: fig-Arginine-refit-resid-qq 
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed weak outliers (the strong ones detected by the 1st pass model have been removed)." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
out_resid_Arginine_mod_refit$outliersPlot
```

```{r}
#| label: fig-Arginine-refit-resid-yhatVsResid
#| fig-cap: "Plot of predictions vs model residuals. No obvious patterns can be seen." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(fitted(Arginine_mod_refit), resid_Arginine_mod_refit, xlab = "Predicted", ylab = "Model Residuals"); abline(h=0)  # yhat vs. r
```

### Variable Importance

```{r}
#| label: fig-Arginine-refit-var-importance
#| fig-cap: "Proportion of overall model $R^2$ that is due to each predictor." 
#| fig-align: left 
#| fig-width: 8   

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(anova(Arginine_mod_refit), what = "proportion R2", main = "", sort = "ascending", margin = "") 
grid(ny = 9, nx = NA, col = "black")
# main = expression(paste("Proportion of overall model ", R^2, " that is due to each predictor"))
# https://library.virginia.edu/data/articles/mathematical-annotation-in-r
# With margin = "partial R2" I believe if you add up all main effect partial R2s and then subtract all interaction effect partial R2s, you get the total R2 as printed from the model object. 
...
<thead>
<tr><td colspan='8' style='text-align: left;'>
Effects          &emsp;&emsp;Response: <code>log(Arginine)</code></td></tr>
<tr><th style='border-bottom: 1px solid grey; font-weight: 900; border-top: 2px solid grey; text-align: center;'></th>
<th style='font-weight: 900; border-bottom: 1px solid grey; border-top: 2px solid grey; text-align: right;'>Low</th>
<th style='font-weight: 900; border-bottom: 1px solid grey; border-top: 2px solid grey; text-align: right;'>High</th>
<th style='font-weight: 900; border-bottom: 1px solid grey; border-top: 2px solid grey; text-align: right;'>&Delta;</th>
...
<td style='text-align: left;'>bmi</td>
<td style='padding-left:4ex; text-align: right;'>22.8</td>
<td style='padding-left:4ex; text-align: right;'> 29.25</td>
<td style='padding-left:4ex; text-align: right;'>  6.45</td>
<td style='padding-left:4ex; text-align: right;'> 0.07149</td>
<td style='padding-left:4ex; text-align: right;'>0.1957</td>
<td style='padding-left:4ex; text-align: right;'>-0.3146</td>
<td style='padding-left:4ex; text-align: right;'> 0.457500</td>
</tr>
<tr>
<td style='text-align: left;'>age</td>
<td style='padding-left:4ex; text-align: right;'>34.0</td>
<td style='padding-left:4ex; text-align: right;'> 53.00</td>
<td style='padding-left:4ex; text-align: right;'> 19.00</td>
<td style='padding-left:4ex; text-align: right;'> 0.57610</td>
<td style='padding-left:4ex; text-align: right;'>0.2176</td>
<td style='padding-left:4ex; text-align: right;'> 0.1468</td>
<td style='padding-left:4ex; text-align: right;'> 1.005000</td>
</tr>
<tr>
<td style='text-align: left;'>resistance_activity</td>
<td style='padding-left:4ex; text-align: right;'> 0.0</td>
<td style='padding-left:4ex; text-align: right;'>540.00</td>
<td style='padding-left:4ex; text-align: right;'>540.00</td>
<td style='padding-left:4ex; text-align: right;'>-0.49740</td>
<td style='padding-left:4ex; text-align: right;'>0.2849</td>
<td style='padding-left:4ex; text-align: right;'>-1.0590</td>
<td style='padding-left:4ex; text-align: right;'> 0.064550</td>
</tr>
<tr>
<td style='text-align: left;'>sex --- Male:Female</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  2.00</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'>-0.26450</td>
<td style='padding-left:4ex; text-align: right;'>0.1357</td>
<td style='padding-left:4ex; text-align: right;'>-0.5322</td>
<td style='padding-left:4ex; text-align: right;'> 0.003239</td>
</tr>
<tr>
<td style='text-align: left;'>supplements_flag --- Yes:No</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  2.00</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'>-0.45350</td>
<td style='padding-left:4ex; text-align: right;'>0.2191</td>
<td style='padding-left:4ex; text-align: right;'>-0.8857</td>
<td style='padding-left:4ex; text-align: right;'>-0.021250</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Thyroidism:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  2.00</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'> 0.15260</td>
<td style='padding-left:4ex; text-align: right;'>0.1962</td>
<td style='padding-left:4ex; text-align: right;'>-0.2343</td>
<td style='padding-left:4ex; text-align: right;'> 0.539500</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Other:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  3.00</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'> 0.17930</td>
<td style='padding-left:4ex; text-align: right;'>0.2243</td>
<td style='padding-left:4ex; text-align: right;'>-0.2630</td>
<td style='padding-left:4ex; text-align: right;'> 0.621700</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Blood:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  4.00</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'> 0.24940</td>
<td style='padding-left:4ex; text-align: right;'>0.2951</td>
<td style='padding-left:4ex; text-align: right;'>-0.3327</td>
<td style='padding-left:4ex; text-align: right;'> 0.831400</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Heart:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  5.00</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'>-0.07056</td>
<td style='padding-left:4ex; text-align: right;'>0.3059</td>
<td style='padding-left:4ex; text-align: right;'>-0.6740</td>
<td style='padding-left:4ex; text-align: right;'> 0.532900</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Stomach:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  6.00</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'> 0.22490</td>
<td style='padding-left:4ex; text-align: right;'>0.3051</td>
<td style='padding-left:4ex; text-align: right;'>-0.3769</td>
<td style='padding-left:4ex; text-align: right;'> 0.826700</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Depression:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  7.00</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'>-0.10230</td>
<td style='padding-left:4ex; text-align: right;'>0.4103</td>
<td style='padding-left:4ex; text-align: right;'>-0.9116</td>
<td style='padding-left:4ex; text-align: right;'> 0.706900</td>
</tr>
<tr>
<td style='border-bottom: 2px solid grey; text-align: left;'>chronic_disease_v3 --- Pain:None</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>  8.00</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'></td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 0.20760</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>0.4638</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.7072</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 1.122000</td>
</tr>
</tbody>
</table>

```{r}
#| label: fig-Arginine-refit-var-effects
#| fig-cap: "The further away the central estimate and its corresponding intervals from unity the higher the variable's effect. Confidence intervals are 90%, 95% and 99%, with the dark blue denoting the 90% confidence interval." 
#| fig-width: 12  

thematic::thematic_off() 
plot(s_Arginine_mod_refit, digits = 1, log = TRUE, main = paste0("% increase/decrease on Arginine reference value of ", round(ddArginine_refit$limits["Arginine"][2,]), " μmol/g (median)"), nb = 14)
```

## Model - 3rd Pass

The 2nd pass model uses too many degrees of freedom (df) in relation to the size of the data i.e. `14` df should be spent instead of the `24` from the model. This can be seen by conducting a validation and calibration analysis. Below are results from the validation analysis using bootstrapping as the resampling technique. The algorithm can be described as follows [source](https://www.nicholas-ollberding.com/post/an-introduction-to-the-harrell-verse-predictive-modeling-using-the-hmisc-and-rms-packages/):
...
# options(prType="html") 
# options(width = 164) 
rms:::html.validate(Arginine_mod_refit_val, digits = 2, caption = "2nd pass model validation")
```
...
<td style='padding-left:3ex; text-align: center;'>0.2</td>
<td style='padding-left:3ex; text-align: center;'>-0.02</td>
<td style='padding-left:3ex; text-align: center;'>293</td>
</tr>
<tr>
<td style='text-align: left;'>MSE</td>
...
<td style='padding-left:3ex; text-align: center;'>0.79</td>
<td style='padding-left:3ex; text-align: center;'>-0.17</td>
<td style='padding-left:3ex; text-align: center;'>0.87</td>
<td style='padding-left:3ex; text-align: center;'>293</td>
</tr>
<tr>
<td style='text-align: left;'>$g$</td>
...
<td style='padding-left:3ex; text-align: center;'>0.53</td>
<td style='padding-left:3ex; text-align: center;'>0.34</td>
<td style='padding-left:3ex; text-align: center;'>0.19</td>
<td style='padding-left:3ex; text-align: center;'>0.25</td>
<td style='padding-left:3ex; text-align: center;'>293</td>
</tr>
<tr>
<td style='text-align: left;'>Intercept</td>
<td style='text-align: center;'>0</td>
<td style='padding-left:3ex; text-align: center;'>0</td>
<td style='padding-left:3ex; text-align: center;'>1.07</td>
<td style='padding-left:3ex; text-align: center;'>-1.07</td>
<td style='padding-left:3ex; text-align: center;'>1.07</td>
<td style='padding-left:3ex; text-align: center;'>293</td>
</tr>
...
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.36</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.64</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>293</td>
</tr>
</tbody>
</table>
<p style="padding-top:2em;">
...
```{r}
#| label: fig-Arginine-refit-cal
#| fig-cap: "Calibration plot for 2nd pass model. Deviation from the diagonal line after correcting for overfitting or bias." 
#| fig-width: 10
 
thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(Arginine_mod_refit_cal)
```

The 2nd pass model also includes the chronic_disease_v3 variable which contains categories with low frequencies in the data. As a result the effect size estimate is accompanied by a very large confidence interval. This renders the estimate as non-trustworthy.  

From these results and also the variable importance plot seen previously as well as the analysis of variance results, it looks like all the considered variables are important, with `bmi` of less importance and with `chronic_disease_v3`'s inclusion being questionable due to high variance. These variables can be kept in a new model with less degree of freedom spending i.e. less overfitting the data. 

Hence the model is refitted to the data including various combinations of these covariates, but excluding the two mentioned above. The selected model spends just `10` degrees of freedom and has a global p-value equal to `0.0007`.(Note: Situation might change when more data are collected and the model is refit).

The chosen model formula is: 

`log(Arginine) ~ sex + rcs(age,4) + log(resistance_activity+1) + supplements_flag + rcs(age,4):log(resistance_activity+1)`

and in expanded form:

```{r}
noquote(reg_f_Arginine_refit2)
```
...
```{r}
#| label: fig-Arginine-refit2-resid-qq 
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed outliers. Here the algorithm reports a single outlier due to its parameterization, however this is a weak outlier since its deviation from the straight line is not large." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
out_resid_Arginine_mod_refit2$outliersPlot
```

```{r}
#| label: fig-Arginine-refit2-resid-yhatVsResid
#| fig-cap: "Plot of predictions vs model residuals. No obvious patterns can be observed." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(fitted(Arginine_mod_refit2), resid_Arginine_mod_refit2, xlab = "Predicted", ylab = "Model Residuals"); abline(h=0)  # yhat vs. r
```

The strong outlier identified in @fig-Arginine-refit2-resid-qq can be further removed from the analysis, and the same formula model is then refit. This leads to (unsurprisingly) a global p-value reduction to `0.0001`, and a residual qq plot without any outliers. 

```{r}
#| label: fig-Arginine-refit3-resid-qq 
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed outliers. Here the algorithm does not reports any strong outliers i.e. outliers whose deviation from the straight line is large." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
out_resid_Arginine_mod_refit3$outliersPlot
```

### Variable Importance

```{r}
#| label: fig-Arginine-refit2-var-importance
#| fig-cap: "Proportion of overall model $R^2$ that is due to each predictor." 
#| fig-align: left 
#| fig-width: 8   

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(anova(Arginine_mod_refit2), what = "proportion R2", main = "", sort = "ascending", margin = "") 
grid(ny = 5, nx = NA, col = "black")
```
...
<td style='padding-left:4ex; text-align: right;'> 53</td>
<td style='padding-left:4ex; text-align: right;'> 19</td>
<td style='padding-left:4ex; text-align: right;'> 0.5520</td>
<td style='padding-left:4ex; text-align: right;'>0.1782</td>
<td style='padding-left:4ex; text-align: right;'> 0.2008</td>
<td style='padding-left:4ex; text-align: right;'> 0.90310</td>
</tr>
<tr>
<td style='text-align: left;'>resistance_activity</td>
<td style='padding-left:4ex; text-align: right;'> 0</td>
<td style='padding-left:4ex; text-align: right;'>540</td>
<td style='padding-left:4ex; text-align: right;'>540</td>
<td style='padding-left:4ex; text-align: right;'>-0.4009</td>
<td style='padding-left:4ex; text-align: right;'>0.2183</td>
<td style='padding-left:4ex; text-align: right;'>-0.8309</td>
<td style='padding-left:4ex; text-align: right;'> 0.02906</td>
</tr>
<tr>
<td style='text-align: left;'>sex --- Male:Female</td>
<td style='padding-left:4ex; text-align: right;'> 1</td>
<td style='padding-left:4ex; text-align: right;'>  2</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'>-0.3929</td>
<td style='padding-left:4ex; text-align: right;'>0.1156</td>
<td style='padding-left:4ex; text-align: right;'>-0.6207</td>
<td style='padding-left:4ex; text-align: right;'>-0.16520</td>
</tr>
<tr>
<td style='border-bottom: 2px solid grey; text-align: left;'>supplements_flag --- Yes:No</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 1</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>  2</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'></td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.1503</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>0.1184</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.3836</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 0.08292</td>
</tr>
</tbody>
</table>

```{r}
#| label: fig-Arginine-refit2-var-effects
#| fig-cap: "The further away the central estimate and its corresponding intervals from unity the higher the variable's effect. Confidence intervals are 90%, 95% and 99%, with the dark blue denoting the 90% confidence interval." 
#| fig-width: 12  

thematic::thematic_off() 
plot(s_Arginine_mod_refit2, digits = 1, log = TRUE, main = paste0("% increase/decrease on Arginine reference value of ", round(ddArginine_refit2$limits["Arginine"][2,]), " μmol/g (median)"), nb = 5) # nb = 4 reduces the spacing between the CIs, looks better without it on the site
```

### Partial Dependence Plots

```{r}
#| label: fig-Arginine-refit2-var-partial-effects-plot1
#| fig-cap: "Effect of sex. Arginine concentration levels can be seen to be higher in females than in males." 
#| fig-width: 8  

ggplot(Predict(Arginine_mod_refit2, sex, fun = exp))
```

```{r}
#| label: fig-Arginine-refit2-var-partial-effects-plot2
#| fig-cap: "Non-Linear effect of age. Age is seen to decrease the levels of Arginine concentration in both age ranges of less than 35 years, as well as more than 55 years. Between 35 and 55 years there seems to be an increasing effect on Arginine concentration."
#| fig-width: 8 

ggplot(Predict(Arginine_mod_refit2, age, fun = exp))
```

```{r}
#| label: fig-Arginine-refit2-var-partial-effects-plot3
#| fig-cap: "Decreasing effect of resistance training on Arginine concentration levels (at a slow pace)."
#| fig-width: 8 

ggplot(Predict(Arginine_mod_refit2, resistance_activity, fun = exp))
```

```{r}
#| label: fig-Arginine-refit2-var-partial-effects-plot4
#| fig-cap: "No effect of supplements_flag on levels of Arginine concentration."
#| fig-width: 8 

ggplot(Predict(Arginine_mod_refit2, supplements_flag, fun = exp))
```

```{r}
#| label: fig-Arginine-refit2-var-partial-effects-plot5
#| fig-cap: "Simultaneous effects of age and resistance_activity. Arginine concentration levels are seen to increase on average with increasing both age and weekly resistance training duration."
#| fig-width: 8  
  
# pred_intx1_Arginine is defined in workspace
bplot(pred_intx1_Arginine, yhat ~ age + resistance_activity, lfun = lattice::wireframe, zlab = "Arginine\n",  screen=list(z=120, x=-75), cex.lab = 0.55, cex.adj = 0.6)
```
...
# options(prType="html") 
# options(width = 164) 
rms:::html.validate(Arginine_mod_refit2_val, digits = 2, caption = "3rd pass model validation")
```
...
<td style='padding-left:3ex; text-align: center;'>0.67</td>
<td style='padding-left:3ex; text-align: center;'>0.72</td>
<td style='padding-left:3ex; text-align: center;'>-0.06</td>
<td style='padding-left:3ex; text-align: center;'>0.75</td>
<td style='padding-left:3ex; text-align: center;'>300</td>
</tr>
<tr>
<td style='text-align: left;'>$g$</td>
...
<td style='text-align: center;'>0</td>
<td style='padding-left:3ex; text-align: center;'>0</td>
<td style='padding-left:3ex; text-align: center;'>0.58</td>
<td style='padding-left:3ex; text-align: center;'>-0.58</td>
<td style='padding-left:3ex; text-align: center;'>0.58</td>
<td style='padding-left:3ex; text-align: center;'>300</td>
</tr>
...
<td style='border-bottom: 2px solid grey; text-align: center;'>1</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>1</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.8</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.2</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.8</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>300</td>
</tr>
</tbody>
...
```{r}
#| label: fig-Arginine-refit2-cal
#| fig-cap: "Calibration plot for 3rd pass model. Deviation from the diagonal line after correcting for overfitting or bias. It can be seen that performance is similar to the 2nd pass model. Main issue is the over-estimation occuring at the right side of the plot, and for larger predicted values." 
#| fig-width: 10
 
thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(Arginine_mod_refit2_cal)
```

### Dynamic Reference Intervals 

The following table shows an example of the construction of a 95% reference interval (RI) for each of four subjects with specific covariate values. For the 3rd pass model only age, sex, resistance_activity and supplements_flag make a difference since the rest of the covariates are not used. 
...
dynRI <- new_dat |>
  nest(.by = id) |>
  mutate(preds = map(data, function(df) predict_f(Arginine_mod_refit2, new_dat = df, conf_type = "mean"))) |>
  unnest_wider(col = preds, names_repair = function(x) {
    nam <- x
    nam[3:4] <- paste0(nam[3:4], "RI")
    nam
  ...
  select(-data) |>
  inner_join(new_dat |> select(id, age, sex, resistance_activity, supplements_flag) |> rename(resistance = resistance_activity, supplements = supplements_flag), by = "id") |>
  mutate(across(where(is.double), ~ round(., 2)))

dynRI  
```

The RI for Arginine from Mayo Clinic Laboratories for ages above 18 years is less than 114 μmol/g of creatinine.

# Glutamine to Glutamate Ratio

## Model - 1st Pass

Test for non-linear relationships between each covariate and the amino acid concentration ratio is executed below. This helps in deciding whether to model an association in a non-linear fashion i.e. with a polynomial term or a spline term or an interaction, or to just consider the covariates separately and without any special transformation. 

```{r}
#| label: fig-ggRatio-nonlinearities 
#| fig-cap: "Spearman's test for assessing non-linear relationships with the response. The further away from 0 the adjusted $\\rho^2$ score is, the higher the degree of non-linearity for that covariate." 
plot(spearman2(ggRatio ~ sex + age + bmi + chronic_disease_flag + chronic_disease_v3 + meds_flag + supplements_flag + aerobics_activity + resistance_activity, data = datGGratio, p = 2, na.action = na.omit))
abline(v = c(-0.01/3, 0.01/3), lty = "dashed")
grid(ny = 9, nx = NA, col = "black")
```

Non-linearity evident in age, bmi, aerobics_activity, resistance_activity, supplements_flag and chronic_disease_v3.

To capture non linearities, interactions amongst these 6 variables will be considered, as well as spline transformations. The model achieving the best global significance (p-value = `0.0666`) contained the following terms: 

`log(ggRatio) ~ rcs(age,4) + rcs(bmi,4) + log(aerobics_activity+1) + log(resistance_activity+1) + supplements_flag + chronic_disease_v3 + rcs(age,4):rcs(bmi,4) + rcs(age,4):log(aerobics_activity+1) + rcs(age,4):log(resistance_activity+1) + rcs(age,4):supplements_flag + rcs(bmi,4):log(aerobics_activity+1) + rcs(bmi,4):log(resistance_activity+1) + rcs(bmi,4):supplements_flag + log(aerobics_activity+1):log(resistance_activity+1) + log(aerobics_activity+1):supplements_flag  + log(resistance_activity+1):supplements_flag`

Where the `:` symbol denotes the interaction between two variables and rcs denotes a restricted cubic spline transformation with 4 knots. ggRatio is the glutamine to glutamate ratio.
...
```{r}
#| label: fig-ggRatio-resid-1stpass 
#| fig-cap: "Figure combining a histogram with a density plot, a boxplot and the plot of the empirical distribution function (ECDF). The scale for the x-axis is synchronized over all plots. The median can be found on the boxplot as also in the ecdf-plot. The maximum and the minimum value are tagged with a tiny vertical dash upon the ecdf-line. The mean is shown in the boxplot as grey cross, the grey bar is its confidence interval." 

thematic::thematic_off()      
DescTools::Desc(resid_ggRatio_mod, plotit = TRUE, digits = 2, main = "Residual Distribution - ggRatio Model")
```

@fig-ggRatio-outliers shows a quantile-quantile plot where the quantiles of the model's residual values are matched to the ones predicted from a normal distribution. The diagonal line indicates the ideal fit. Data points marked in red denote outliers (also indicated in the table below each plot with their corresponding subject IDs). 

```{r}
#| label: fig-ggRatio-outliers 
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed outliers." 

# thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
out_resid_ggRatio_mod$outliersPlot
```

```{r}
gt(out_resid_ggRatio_mod$outliers) |> 
  tab_header(
    title = md("**Outliers**")
  ) |>  
  fmt_number(columns = ggRatio) |> 
  tab_footnote(
    footnote = "Red coloured data points in above QQ plot",
    locations = cells_column_labels(columns = id)
  ) |> 
  tab_footnote(
    footnote = "concentration ratio (model residual)",
    locations = cells_column_labels(columns = ggRatio)
  ) |> 
  tab_options(table.background.color = "lavenderblush3", 
              table.font.size = "12px", 
              table.font.style = "oblique") |> 
...
Here the model is refitted to the data excluding the strong outlier values detected in the previous step. The model formula is the same as the 1st pass. The global regression significance is characterized by a p-value of `0.0151`. 

### Residuals

```{r}
#| label: fig-ggRatio-refit-resid-qq 
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed weak outliers (the strong ones detected by the 1st pass model have been removed)." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
out_resid_ggRatio_mod_refit$outliersPlot
```

```{r}
#| label: fig-ggRatio-refit-resid-yhatVsResid
#| fig-cap: "Plot of predictions vs model residuals. No obvious patterns can be seen." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(fitted(ggRatio_mod_refit), resid_ggRatio_mod_refit, xlab = "Predicted", ylab = "Model Residuals"); abline(h=0)  # yhat vs. r
```

### Variable Importance

```{r}
#| label: fig-ggRatio-refit-var-importance
#| fig-cap: "Proportion of overall model $R^2$ that is due to each predictor." 
#| fig-align: left 
#| fig-width: 8   

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(anova(ggRatio_mod_refit), what = "proportion R2", main = "", sort = "ascending", margin = "") 
grid(ny = 16, nx = NA, col = "black")
```
...
<thead>
<tr><td colspan='8' style='text-align: left;'>
Effects          &emsp;&emsp;Response: <code>log(ggRatio)</code></td></tr>
<tr><th style='border-bottom: 1px solid grey; font-weight: 900; border-top: 2px solid grey; text-align: center;'></th>
<th style='font-weight: 900; border-bottom: 1px solid grey; border-top: 2px solid grey; text-align: right;'>Low</th>
<th style='font-weight: 900; border-bottom: 1px solid grey; border-top: 2px solid grey; text-align: right;'>High</th>
<th style='font-weight: 900; border-bottom: 1px solid grey; border-top: 2px solid grey; text-align: right;'>&Delta;</th>
...
<td style='padding-left:4ex; text-align: right;'> 53.0</td>
<td style='padding-left:4ex; text-align: right;'> 19.0</td>
<td style='padding-left:4ex; text-align: right;'>-0.699400</td>
<td style='padding-left:4ex; text-align: right;'>0.2642</td>
<td style='padding-left:4ex; text-align: right;'>-1.22100</td>
<td style='padding-left:4ex; text-align: right;'>-0.17810</td>
</tr>
<tr>
<td style='text-align: left;'>bmi</td>
<td style='padding-left:4ex; text-align: right;'>22.8</td>
<td style='padding-left:4ex; text-align: right;'> 29.1</td>
<td style='padding-left:4ex; text-align: right;'>  6.3</td>
<td style='padding-left:4ex; text-align: right;'> 0.008315</td>
<td style='padding-left:4ex; text-align: right;'>0.2199</td>
<td style='padding-left:4ex; text-align: right;'>-0.42560</td>
<td style='padding-left:4ex; text-align: right;'> 0.44220</td>
</tr>
<tr>
<td style='text-align: left;'>aerobics_activity</td>
<td style='padding-left:4ex; text-align: right;'> 0.0</td>
<td style='padding-left:4ex; text-align: right;'> 60.0</td>
<td style='padding-left:4ex; text-align: right;'> 60.0</td>
<td style='padding-left:4ex; text-align: right;'> 0.407600</td>
<td style='padding-left:4ex; text-align: right;'>0.2281</td>
<td style='padding-left:4ex; text-align: right;'>-0.04250</td>
<td style='padding-left:4ex; text-align: right;'> 0.85770</td>
</tr>
<tr>
<td style='text-align: left;'>resistance_activity</td>
<td style='padding-left:4ex; text-align: right;'> 0.0</td>
<td style='padding-left:4ex; text-align: right;'>540.0</td>
<td style='padding-left:4ex; text-align: right;'>540.0</td>
<td style='padding-left:4ex; text-align: right;'>-0.360700</td>
<td style='padding-left:4ex; text-align: right;'>0.4512</td>
<td style='padding-left:4ex; text-align: right;'>-1.25100</td>
<td style='padding-left:4ex; text-align: right;'> 0.52960</td>
</tr>
<tr>
<td style='text-align: left;'>supplements_flag --- Yes:No</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  2.0</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'> 0.256000</td>
<td style='padding-left:4ex; text-align: right;'>0.2314</td>
<td style='padding-left:4ex; text-align: right;'>-0.20070</td>
<td style='padding-left:4ex; text-align: right;'> 0.71270</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Thyroidism:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  2.0</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'> 0.416000</td>
<td style='padding-left:4ex; text-align: right;'>0.1807</td>
<td style='padding-left:4ex; text-align: right;'> 0.05930</td>
<td style='padding-left:4ex; text-align: right;'> 0.77260</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Other:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  3.0</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'> 0.336600</td>
<td style='padding-left:4ex; text-align: right;'>0.2089</td>
<td style='padding-left:4ex; text-align: right;'>-0.07561</td>
<td style='padding-left:4ex; text-align: right;'> 0.74870</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Blood:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  4.0</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'> 0.152600</td>
<td style='padding-left:4ex; text-align: right;'>0.2796</td>
<td style='padding-left:4ex; text-align: right;'>-0.39910</td>
<td style='padding-left:4ex; text-align: right;'> 0.70430</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Heart:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  5.0</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'> 0.265000</td>
<td style='padding-left:4ex; text-align: right;'>0.2960</td>
<td style='padding-left:4ex; text-align: right;'>-0.31920</td>
<td style='padding-left:4ex; text-align: right;'> 0.84910</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Stomach:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  6.0</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'> 0.274300</td>
<td style='padding-left:4ex; text-align: right;'>0.2817</td>
<td style='padding-left:4ex; text-align: right;'>-0.28160</td>
<td style='padding-left:4ex; text-align: right;'> 0.83020</td>
</tr>
<tr>
<td style='text-align: left;'>chronic_disease_v3 --- Depression:None</td>
<td style='padding-left:4ex; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; text-align: right;'>  7.0</td>
<td style='padding-left:4ex; text-align: right;'></td>
<td style='padding-left:4ex; text-align: right;'>-0.614900</td>
<td style='padding-left:4ex; text-align: right;'>0.3611</td>
<td style='padding-left:4ex; text-align: right;'>-1.32800</td>
<td style='padding-left:4ex; text-align: right;'> 0.09778</td>
</tr>
<tr>
<td style='border-bottom: 2px solid grey; text-align: left;'>chronic_disease_v3 --- Pain:None</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 1.0</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>  8.0</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'></td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.146200</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>0.4237</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.98220</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 0.68980</td>
</tr>
</tbody>
</table>

```{r}
#| label: fig-ggRatio-refit-var-effects
#| fig-cap: "The further away the central estimate and its corresponding intervals from unity the higher the variable's effect. Confidence intervals are 90%, 95% and 99%, with the dark blue denoting the 90% confidence interval." 
#| fig-width: 12  

thematic::thematic_off() 
plot(s_ggRatio_mod_refit, digits = 1, log = TRUE, main = paste0("% increase/decrease on ggRatio reference value of ", round(ddGGratio_refit$limits["ggRatio"][2,]), " (median)"), nb = 13)
```

## Model - 3rd Pass

The 2nd pass model uses too many degrees of freedom (df) in relation to the size of the data i.e. `15` df should be spent instead of the `46` from the model. This can be seen by conducting a validation and calibration analysis. Below are results from the validation analysis using bootstrapping as the resampling technique. The algorithm can be described as follows [source](https://www.nicholas-ollberding.com/post/an-introduction-to-the-harrell-verse-predictive-modeling-using-the-hmisc-and-rms-packages/):
...
# options(prType="html") 
# options(width = 164) 
rms:::html.validate(ggRatio_mod_refit_val, digits = 2, caption = "2nd pass model validation")
```
...
<td style='padding-left:3ex; text-align: center;'>0.48</td>
<td style='padding-left:3ex; text-align: center;'>-0.22</td>
<td style='padding-left:3ex; text-align: center;'>295</td>
</tr>
<tr>
<td style='text-align: left;'>MSE</td>
...
<td style='padding-left:3ex; text-align: center;'>0.67</td>
<td style='padding-left:3ex; text-align: center;'>-0.3</td>
<td style='padding-left:3ex; text-align: center;'>0.76</td>
<td style='padding-left:3ex; text-align: center;'>295</td>
</tr>
<tr>
<td style='text-align: left;'>$g$</td>
...
<td style='text-align: center;'>0</td>
<td style='padding-left:3ex; text-align: center;'>0</td>
<td style='padding-left:3ex; text-align: center;'>1.56</td>
<td style='padding-left:3ex; text-align: center;'>-1.56</td>
<td style='padding-left:3ex; text-align: center;'>1.56</td>
<td style='padding-left:3ex; text-align: center;'>295</td>
</tr>
...
<td style='border-bottom: 2px solid grey; text-align: center;'>1</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>1</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.47</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.53</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.47</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>295</td>
</tr>
</tbody>
</table>
<p style="padding-top:2em;">
...
```{r}
#| label: fig-ggRatio-refit-cal
#| fig-cap: "Calibration plot for 2nd pass model. Deviation from the diagonal line after correcting for overfitting or bias." 
#| fig-width: 10
 
thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(ggRatio_mod_refit_cal)
```

The 2nd pass model also includes the chronic_disease_v3 variable which contains categories with low frequencies in the data. As a result the effect size estimate is accompanied by a very large confidence interval. This renders the estimate as non-trustworthy.  

From these results and also the variable importance plot seen previously as well as the analysis of variance results, it looks like all the `age` and `bmi` are important variables to be kept in the model, as well as their interaction. `chronic_disease_v3`'s inclusion is questionable due to high variance. `resistance_activity` also does seem to associate to the ratio respomse but a lesser degree. These variables can be kept in a new model with less degree of freedom spending i.e. less overfitting the data. 

Hence the model is refitted to the data including various combinations of these covariates, but excluding the two mentioned above. The selected model spends `20` degrees of freedom and has a global p-value equal to `0.0428`. (Note: Situation might change when more data are collected and the model is refit).

The chosen model formula is: 

`log(ggRatio) ~ rcs(age,5) + rcs(bmi,4) + log(resistance_activity+1) + rcs(age,5):rcs(bmi,4)`

Where the `:` symbol denotes the interaction between two variables and rcs denotes a restricted cubic spline transformation with 4 knots for `bmi` and 5 knots for `age`.
...
```{r}
#| label: fig-ggRatio-refit2-resid-qq 
#| fig-cap: "Normal probabibilty plot comparing quantiles of model residuals and the theoretical normal distribution. Vertical dashed lines indicate the central 95% limits of the normal distribution. Horizontal dashed lines indicate outlier detection thresholds i.e. data outside of these thresholds are deemed outliers. Here the algorithm reports a single outlier due to its parameterization, however this is a weak outlier since its deviation from the straight line is not large." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
out_resid_ggRatio_mod_refit2$outliersPlot
```

```{r}
#| label: fig-ggRatio-refit2-resid-yhatVsResid
#| fig-cap: "Plot of predictions vs model residuals. No obvious patterns can be observed." 

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(fitted(ggRatio_mod_refit2), resid_ggRatio_mod_refit2, xlab = "Predicted", ylab = "Model Residuals"); abline(h=0)  # yhat vs. r
```

### Variable Importance

```{r}
#| label: fig-ggRatio-refit2-var-importance
#| fig-cap: "Proportion of overall model $R^2$ that is due to each predictor." 
#| fig-align: left 
#| fig-width: 8   

thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(anova(ggRatio_mod_refit2), what = "proportion R2", main = "", sort = "ascending", margin = "") 
grid(ny = 4, nx = NA, col = "black")
```
...
<td style='padding-left:4ex; text-align: right;'> 53.0</td>
<td style='padding-left:4ex; text-align: right;'> 19.0</td>
<td style='padding-left:4ex; text-align: right;'>-0.2574</td>
<td style='padding-left:4ex; text-align: right;'>0.2086</td>
<td style='padding-left:4ex; text-align: right;'>-0.66870</td>
<td style='padding-left:4ex; text-align: right;'>0.15390</td>
</tr>
<tr>
<td style='text-align: left;'>bmi</td>
...
<td style='padding-left:4ex; text-align: right;'> 29.1</td>
<td style='padding-left:4ex; text-align: right;'>  6.3</td>
<td style='padding-left:4ex; text-align: right;'> 0.4163</td>
<td style='padding-left:4ex; text-align: right;'>0.2430</td>
<td style='padding-left:4ex; text-align: right;'>-0.06273</td>
<td style='padding-left:4ex; text-align: right;'>0.89540</td>
</tr>
<tr>
<td style='border-bottom: 2px solid grey; text-align: left;'>resistance_activity</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'> 0.0</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>540.0</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>540.0</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.2633</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>0.1574</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>-0.57370</td>
<td style='padding-left:4ex; border-bottom: 2px solid grey; text-align: right;'>0.04699</td>
</tr>
</tbody>
</table>

```{r}
#| label: fig-ggRatio-refit2-var-effects
#| fig-cap: "The further away the central estimate and its corresponding intervals from unity the higher the variable's effect. Confidence intervals are 90%, 95% and 99%, with the dark blue denoting the 90% confidence interval." 
#| fig-width: 12  

thematic::thematic_off() 
plot(s_ggRatio_mod_refit2, digits = 1, log = TRUE, main = paste0("% increase/decrease on ggRatio reference value of ", round(ddGGratio_refit$limits["ggRatio"][2,]), " (median)"), nb = 4) # nb = 4 reduces the spacing between the CIs, looks better without it on the site
```

### Partial Dependence Plots

```{r}
#| label: fig-ggRatio-refit2-var-partial-effects-plot1
#| fig-cap: "Non-Linear effect of age. Age is seen to slowly increase the levels of the Glutamine/Glutamate ratio in the range of older than 50 years." 
#| fig-width: 8  

ggplot(Predict(ggRatio_mod_refit2, age, fun = exp))
```

```{r}
#| label: fig-ggRatio-refit2-var-partial-effects-plot2
#| fig-cap: "Non-Linear effect of bmi. BMI is seen to decrease the levels of the Glutamine/Glutamate ratio in the range of larger than 30 $kg/m^2$."
#| fig-width: 8 

ggplot(Predict(ggRatio_mod_refit2, bmi, fun = exp))
```

```{r}
#| label: fig-ggRatio-refit2-var-partial-effects-plot3
#| fig-cap: "Decreasing effect of resistance training on the Glutamine/Glutamate ratio levels (at a slow pace)."
#| fig-width: 8 

ggplot(Predict(ggRatio_mod_refit2, resistance_activity, fun = exp))
```

```{r}
#| label: fig-ggRatio-refit2-var-partial-effects-plot5
#| fig-cap: "Simultaneous effects of age and bmi. Two peaks of the Glutamine/Glutamate ratio levels can be seen. One corresponds to an age of 30 years and a bmi of 45 $kg/m^2$, and the larger peak corresponds to very young individuals i.e. below 20 years, and a bmi value of around 30 $kg/m^2$."
#| fig-width: 8  
  
# pred_intx1_ggRatio is defined in workspace
bplot(pred_intx1_ggRatio, yhat ~ age + bmi, lfun = lattice::wireframe, zlab = "ggRatio\n",  screen=list(z=150, x=-75), cex.lab = 0.55, cex.adj = 0.6, ylabrot = 40)
```
...
# options(prType="html") 
# options(width = 164) 
rms:::html.validate(ggRatio_mod_refit2_val, digits = 2, caption = "3rd pass model validation")
```
...
<td style='padding-left:3ex; text-align: center;'>0.64</td>
<td style='padding-left:3ex; text-align: center;'>-0.14</td>
<td style='padding-left:3ex; text-align: center;'>0.68</td>
<td style='padding-left:3ex; text-align: center;'>300</td>
</tr>
<tr>
<td style='text-align: left;'>$g$</td>
...
<td style='text-align: center;'>0</td>
<td style='padding-left:3ex; text-align: center;'>0</td>
<td style='padding-left:3ex; text-align: center;'>1.35</td>
<td style='padding-left:3ex; text-align: center;'>-1.35</td>
<td style='padding-left:3ex; text-align: center;'>1.35</td>
<td style='padding-left:3ex; text-align: center;'>300</td>
</tr>
...
<td style='border-bottom: 2px solid grey; text-align: center;'>1</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>1</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.54</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.46</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>0.54</td>
<td style='padding-left:3ex; border-bottom: 2px solid grey; text-align: center;'>300</td>
</tr>
</tbody>
...
```{r}
#| label: fig-ggRatio-refit2-cal
#| fig-cap: "Calibration plot for 3rd pass model. Deviation from the diagonal line after correcting for overfitting or bias. It can be seen that performance has improved compared to the 2nd pass model. Main issue that is still remaining is the under-estimation occuring at the left side of the plot, and for smaller predicted ratios." 
#| fig-width: 10
 
thematic::thematic_on(bg = "lavenderblush3", fg = "black", accent = "blue") 
plot(ggRatio_mod_refit2_cal)
```

### Dynamic Reference Intervals 

The following table shows an example of the construction of a 95% reference interval (RI) for each of four subjects with specific covariate values. For the 3rd pass model only age, bmi and resistance_activity make a difference since the rest of the covariates are not used. 
...
dynRI <- new_dat |>
  nest(.by = id) |>
  mutate(preds = map(data, function(df) predict_f(ggRatio_mod_refit2, new_dat = df, conf_type = "mean"))) |>
  unnest_wider(col = preds, names_repair = function(x) {
    nam <- x
    nam[3:4] <- paste0(nam[3:4], "RI")
    nam
  ...
  select(-data) |>
  inner_join(new_dat |> select(id, age, bmi, resistance_activity) |> rename(resistance = resistance_activity), by = "id") |>
  mutate(across(where(is.double), ~ round(., 2)))
